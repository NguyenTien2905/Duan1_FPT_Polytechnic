...
$nameErr = $emailErr = $descErr = $priceErr = $imgErr = $quanErr = $emailErr = $passErr = $userErr = $teleErr= "";
if (isset($_GET['act'])) {
    $act = $_GET['act'];
    switch ($act) {
            ...
            if ($_SESSION['user']['permissions'] == 1) {
                if (isset($_POST['update']) && $_POST['update']) {
                    ...
                    if (!preg_match('/^[a-zA-Z0-9\s]+$/', $user)) {
                        ...
                        $descErr = "Invalid email format";
                        $noti = "Failed";
                    } else if (preg_match('/^0/', $number_phone)) {
                        $teleErr = "Phone number format is not standard";
                        $noti = "Failed";
                    } else if ($_FILES['image']['size'] > (1024 * 1024 * 15)) {
                        $imgErr = "Image size must not exceed 15 MB";
                        $noti = "Failed";
